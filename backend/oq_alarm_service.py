...
def get_available_machine_types():
    """
    Returns list of available machine types inside GMP_Alarme directory.
    """
    if not os.path.exists(GMP_ALARME_BASE):
        return ["FP", "SC_SI", "TZC", "FC", "TL"]
    types = [d for d in os.listdir(GMP_ALARME_BASE) if os.path.isdir(os.path.join(GMP_ALARME_BASE, d)) and not d.startswith('.')]
    return sorted(types) if types else ["FP", "SC_SI", "TZC", "FC", "TL"]

def create_machine_type(machine_name):
    """
    Creates a new machine type directory structure under GMP_Alarme and .docx_cache.
    ...
    src_mach_dir = os.path.join(GMP_ALARME_BASE, clean_name)
    ...
    # Copy default Performer template from FP or available machine types
    default_perf_doc = os.path.join(GMP_ALARME_BASE, "FP", "EN", "Performer.doc")
    default_perf_docx = os.path.join(DOCX_CACHE_BASE, "FP", "EN", "Performer.docx")
    
    if os.path.exists(default_perf_doc):
        shutil.copy2(default_perf_doc, os.path.join(src_dir, "Performer.doc"))
    if os.path.exists(default_perf_docx):
        shutil.copy2(default_perf_docx, os.path.join(src_dir, "Performer.docx"))
        shutil.copy2(default_perf_docx, os.path.join(cache_dir, "Performer.docx"))
        
    return clean_name

def get_performer_table_xml(machine_type="FP", lang="EN"):
    """
    Robustly locates, synchronizes, and returns the Performer sign-off table XML.
    Uses multi-level fallback across cache, current machine type, and standard templates.
    """
    cache_dir = os.path.join(DOCX_CACHE_BASE, machine_type, lang)
    src_dir = os.path.join(GMP_ALARME_BASE, machine_type, lang)
    os.makedirs(cache_dir, exist_ok=True)
    ...
    # 3. Search fallbacks from other machine types (FP, FC, SC_SI, TL, TZC, etc.)
    fallback_sources = [
        os.path.join(DOCX_CACHE_BASE, "FP", "EN", "Performer.docx"),
        os.path.join(DOCX_CACHE_BASE, "FC", "EN", "Performer.docx"),
        os.path.join(DOCX_CACHE_BASE, "SC_SI", "EN", "Performer.docx"),
        os.path.join(DOCX_CACHE_BASE, "TL", "EN", "Performer.docx"),
        os.path.join(GMP_ALARME_BASE, "FP", "EN", "Performer.docx"),
    ]
    if os.path.exists(DOCX_CACHE_BASE):
        for d in os.listdir(DOCX_CACHE_BASE):
            cand = os.path.join(DOCX_CACHE_BASE, d, "EN", "Performer.docx")
            if cand not in fallback_sources:
                fallback_sources.append(cand)

    for fb_path in fallback_sources:
...
def delete_machine_type(machine_name):
    """
    Safely deletes a machine type directory from GMP_Alarme and .docx_cache.
    """
...
def list_machine_files_detailed(machine_type, lang="EN"):
    """
    Returns a pandas DataFrame of all template files in GMP_Alarme/{machine_type}/{lang}/.
...
def upload_machine_files(machine_type, uploaded_files, lang="EN"):
    """
    Saves uploaded files (.doc, .docx) into GMP_Alarme/{machine_type}/{lang}/ and syncs .docx_cache.
...
def delete_machine_files(machine_type, filenames, lang="EN"):
    """
    Deletes specified files from GMP_Alarme/{machine_type}/{lang}/ and their cache counterparts.
    """
    src_dir = os.path.join(GMP_ALARME_BASE, machine_type, lang)
    cache_dir = os.path.join(DOCX_CACHE_BASE, machine_type, lang)
...
def get_machine_file_bytes(machine_type, filename, lang="EN"):
    ...
    """
    src_fp = os.path.join(GMP_ALARME_BASE, machine_type, lang, filename)
    if os.path.exists(src_fp):
...
def get_available_alarm_templates(machine_type="FP", lang="EN"):
    """
    Returns list of available alarm template files for a given machine type and language.
    """
    cache_dir = os.path.join(DOCX_CACHE_BASE, machine_type, lang)
...
def ensure_docx_cache(machine_type="FP", lang="EN"):
    ...
    Also synchronizes any .docx files from GMP_Alarme and ensures Performer.docx is present.
    """
    src_dir = os.path.join(GMP_ALARME_BASE, machine_type, lang)
...
def parse_and_match_alarms(excel_source, machine_type="FP", lang="EN"):
    """
    Parses Alarm Excel, extracts Column A (Variable) and Column E (Alarmtext_Englisch),
    and looks up matching alarm templates in GMP_Alarme/{machine_type}/{lang}/.
    """
    # Load Excel
    ...
    sheet_name = "Messages" if "Messages" in wb.sheetnames else wb.sheetnames[0]
    ws = wb[sheet_name]
    ...
    for r in range(1, min(6, ws.max_row + 1)):
        for c in range(1, min(10, ws.max_column + 1)):
            ...
            if 'variable' in v:
                col_var_idx = c
                header_row = r
            elif 'alarmtext_englisch' in v or 'english' in v:
                col_en_idx = c
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        # Extract alarm code MX_...
        m = re.search(r'(MX_[A-Za-z0-9_]+)', var_val, re.IGNORECASE)
        alarm_code = m.group(1) if m else var_val

        # Match against available alarm docx
        matched_file = available_docx.get(alarm_code.lower())
...
def generate_matched_alarms_excel(matched_records, base_name="5XXXX"):
    """
    Generates summary Excel containing Column A (No.), Column B (Variable), and Column C (Alarmtext_Englisch).
    """
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Matched Alarms"

    # Header
    headers = ["No.", "Variable", "Alarmtext_Englisch"]
    ws.append(headers)
    ...
    for idx, r in enumerate(matched_records, 1):
        ws.append([idx, r["Variable"], r["Alarmtext_Englisch"]])
    ...
    excel_filename = f"{base_name}_Matched_Alarms_Summary.xlsx"
    return doc_io.getvalue(), excel_filename

def generate_oq_alarm_word(
    excel_source,
    machine_type="FP",
    ...
    **kwargs
    ...
    """
    Generates OQ Alarms (GMP Alarms) Word Protocol:
    1. Matches alarms against GMP_Alarme/{machine_type}/{lang}/.
    2. Opens Master Template and locates 'InsertTests' or 'XXXX' in Section 3.
    3. Merges alarm tables and Performer tables consecutively.
    4. Generates summary Excel of matched alarms.
    5. Returns doc_bytes, summary_excel_bytes, and result metadata.
    """
    parsed = parse_and_match_alarms(excel_source, machine_type, lang)
    matched_records = parsed["matched_records"]

    if not matched_records:
        raise ValueError(f"No matching alarm templates found in {machine_type} for the provided Excel alarm variables.")
    ...
    # Insert all matched alarms
    for idx, item in enumerate(matched_records):
        alarm_file = item["File Path"]
        if not alarm_file or not os.path.exists(alarm_file):
            continue

        alarm_doc = docx.Document(alarm_file)
        if not alarm_doc.tables:
            continue

        # Clone and insert alarm table
        alarm_tbl_elem = copy.deepcopy(alarm_doc.tables[0]._tbl)
        target_p._p.addprevious(alarm_tbl_elem)
        ...
        # Insert page break between alarm tests (except after the last one)
    ...
    if base_name == "XXXXX" or not base_name:
        base_name = "5XXXX"
    excel_bytes, excel_name = generate_matched_alarms_excel(matched_records, base_name)
